...
export default function Login() {
  ...
  // Check if user just came from register page
  const cameFromRegister = location.state?.registered === true;
  ...
  const handleSubmit = async (e) => {
    ...
    // Step 1: Check if account exists first
    try {
      const checkRes = await fetch("http://127.0.0.1:8000/check-email", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email: email.trim() }),
      });

      // If backend has a check-email endpoint
      if (checkRes.ok) {
        const checkData = await checkRes.json();
        if (!checkData.exists) {
          return setLocalErr("No account found with this email. Please create one.");
        }
      }
    } catch {
      // No check-email endpoint — skip this step and rely on login response
    }
    ...
    if (!result.success) {
      ...
      if (
        ...
        setLocalErr("No account found with this email. Please create one.");
      } else if (
        ...
        setLocalErr(result.error || "Login failed. Please try again.");
      }
    } else {
  ...
  return (
    <div className="min-h-screen relative overflow-hidden flex items-center justify-center px-4">
      ...
      <div className="fixed top-[-150px] left-[-150px] w-[500px] h-[500px] rounded-full bg-blue-200/30 blur-3xl" />
      <div className="fixed bottom-[-150px] right-[-150px] w-[500px] h-[500px] rounded-full bg-indigo-200/30 blur-3xl" />
      ...
      <div className="relative z-10 w-full max-w-md fade-in">
        ...
        {/* Success message if came from register */}
        {cameFromRegister && (
          <div className="flex items-center gap-3 bg-emerald-50 border border-emerald-200 rounded-2xl px-4 py-3 mb-4 fade-in">
            <svg className="w-4 h-4 text-emerald-500 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
            </svg>
            <p className="text-xs text-emerald-700 font-medium">Account created! Sign in to continue.</p>
          </div>
        )}
            ...
            {displayError && (
              <div className="flex items-start gap-3 bg-red-50 border border-red-200 rounded-2xl px-4 py-3 fade-in">
                ...
                <div>
                  ...
                  {(displayError.includes("No account") || displayError.includes("create one")) && (
                    <Link to="/register" className="text-xs text-indigo-500 font-semibold hover:text-indigo-700 mt-1 inline-block">
                      Create an account →
                    </Link>
                  )}
                </div>
